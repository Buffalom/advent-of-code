...
const coords1 = input.map(inst => inst.split(' '))
  .reduce((acc, [dir, amount]) => {
    switch (dir) {
      case 'forward':
        acc.x += parseInt(amount)
        break
      case 'up':
        acc.z -= parseInt(amount)
        break
      case 'down':
        acc.z += parseInt(amount)
        break
...
const result1 = coords1.x * coords1.z
...
const coords2 = input.map(inst => inst.split(' '))
  .reduce((acc, [dir, amount]) => {
    switch (dir) {
      case 'forward':
        acc.x += parseInt(amount)
        acc.z += acc.aim * parseInt(amount)
        break
      case 'up':
...
const result2 = coords2.x * coords2.z
