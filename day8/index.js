...
function work(inputs, acc = 0, workingIndex = 0, visited = []) {
  ...
  return work(inputs, acc, workingIndex, visited)
}
 ...
 */
console.log('PART 1')
let [loopedAcc] = work(inputs)
console.log(loopedAcc)

/*
 * PART 2
 */
function bruteforceBugfix(inputs) {
  for (let i in inputs) {
    const tmpInputs = [...inputs]
    const match = tmpInputs[i].match(/^(jmp|nop)/)

    if (Array.isArray(match)) {
      if (match[1] === 'jmp') {
        tmpInputs[i] = tmpInputs[i].replace('jmp', 'nop')
      } else {
        tmpInputs[i] = tmpInputs[i].replace('nop', 'jmp')
      }
      let [acc, end] = work(tmpInputs)
      if (end === 'finished') {
        return [acc, end]
      }
    }
...
console.log('PART 2')
let [finalAcc] = bruteforceBugfix(inputs)
