...
function findSummands(countOfSummands, target, nums) {
  ...
  for (const num of nums) {
    if (countOfSummands > 2) {
      ...
      const found = findOtherPartForSum(num, target, prevInputs)
      if (found) {
        sumOfTheseIsTargetSum.push(found)
        sumOfTheseIsTargetSum.push(num)
        break
      }
      else {
 ...
 */
console.log('PART 1')
console.log(findSummands(2, targetSum, inputs).reduce((acc, curr) => acc * curr))

/*
 * PART 2
 */
console.log('PART 2')
console.log(findSummands(3, targetSum, inputs).reduce((acc, curr) => acc * curr))
