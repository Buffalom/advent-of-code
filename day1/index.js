...
function findSummandsRefactor(countOfSummands, target, nums) {
  const sumOfTheseIsTargetSum = []

  if (countOfSummands > 2) {
    nums.some((num, i) => {
      const others = findSummandsRefactor(countOfSummands - 1, target - num, nums.slice(i + 1))
      if (others.length) {
        sumOfTheseIsTargetSum.push(...[num, ...others])
        return true
      }
    })
  } else {
    nums.some((num, i) => {
      const otherSummand = nums.slice(i + 1).find((other) => (other + num) === target)
      if (otherSummand) {
        sumOfTheseIsTargetSum.push(num)
        sumOfTheseIsTargetSum.push(otherSummand)
        return true
      }
    })
  }
 ...
 */
console.log('PART 1')
console.log(findSummandsRefactor(2, targetSum, inputs).reduce((acc, curr) => acc * curr))

/*
 * PART 2
 */
console.log('PART 2')
console.log(findSummandsRefactor(3, targetSum, inputs).reduce((acc, curr) => acc * curr))
