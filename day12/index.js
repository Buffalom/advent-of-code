...
function turn(facing, dir, amount) {
  ...
  index = index < 0 ? index + directions.length : index % directions.length

  return directions[index]
}

function rotate(relPos, dir, amount) {
  steps = amount / 90
  if (steps > 1) {
    return (new Array(steps)).fill(null).reduce((relPos) => rotate(relPos, dir, 90), relPos)
  } else if (dir === 'R') {
    return {
      x: relPos.y,
      y: -relPos.x,
 ...
 * PART 1
 */
function travelWithWrongInstructions(inputs, facing = 'E', pos = { x: 0, y: 0 }) {
  return inputs.reduce((pos, input) => {
    let [_, instruction, amount] = input.match(/^([NESWFRL])(\d+)$/)
    amount = parseInt(amount)

    if (directions.includes(instruction)) {
      let moved = move(instruction, amount)
      pos.x += moved.x
      pos.y += moved.y
    } else if (instruction === 'F') {
      let moved = move(facing, amount)
      pos.x += moved.x
      pos.y += moved.y
    } else if (['R', 'L'].includes(instruction)) {
      facing = turn(facing, instruction, amount)
    }
...
console.log('PART 1')
const pos = travelWithWrongInstructions(inputs)
console.log(Math.abs(pos.x) + Math.abs(pos.y))

/*
 * PART 2
 */
function travelWithRightInstructions(inputs, wayPos = { x: 0, y: 0 }, pos = { x: 0, y: 0 }) {
  return inputs.reduce((pos, input) => {
    let [_, instruction, amount] = input.match(/^([NESWFRL])(\d+)$/)
    amount = parseInt(amount)
    ...
    if (directions.includes(instruction)) {
      ...
      pos.x += wayPos.x * amount
      pos.y += wayPos.y * amount
    } else if (['R', 'L'].includes(instruction)) {
      wayPos = rotate(wayPos, instruction, amount)
    }
...
console.log('PART 2')
const pos2 = travelWithRightInstructions(inputs, { x: 10, y: 1 })
console.log(Math.abs(pos2.x) + Math.abs(pos2.y))
